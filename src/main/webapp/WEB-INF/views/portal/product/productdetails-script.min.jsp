...
function getTogetherData(callback) {
	$.ajax({
		...
		data: JSON.stringify({ "producttogetherId": proudctTogetherId }),
		dataType: 'json',
...
function renderTogetherData(data) {
	var productDatas = data.mlbackProductList,
		...
		productSkuProps = data.propAttributeNameListList,
		len = productDatas.length;
	function renderProductTogetherSKus(data) {
		var optionHtml = '';
		data.forEach(function(item, idx) {
			optionHtml += '<option data-id="'+ item.productskuId +'" data-prop="'+ ('') +'" data-code="'+ item.productskuCode +'" data-price="'+ item.productskuMoney +'" data-name="'+ item.productskuName +'" value="'+ item.productskuName +'">'+ item.productskuName +'</option>';
		});
		return optionHtml;
	}
	if (data.mlbackProductTogetherRes && len && productSkuDatas.length && productDatas.length == productSkuDatas.length && productSkuProps.length == productSkuDatas.length) {
		...
		productDatas.forEach(function(item, idx) {
			...
			otherHtml += '<div class="product-together-info-item" data-dis="'+ item.productActoffoff +'" data-price="'+ item.productOriginalprice +'" data-id="'+ item.productId +'">' +
				...
				'<select class="product-together-select">' + renderProductTogetherSKus(productSkuDatas[idx]) + '</select>' +
